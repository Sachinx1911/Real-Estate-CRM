...
- **सध्या वगळलेले (v2 साठी — final ठरले):** WhatsApp/SMS/Email notifications, Broker/channel partner, Commission tracking, Site-visit scheduling, Portal integrations (99acres/MagicBricks/FB), Expense tracking, **उशिरा हप्त्यावर व्याज/penalty** — v1 (Phase 0–13) पूर्ण झाल्यावरच यांचा विचार
...
3. **properties** — units (flat/shop/office)
   - id, project_id, wing, floor, flat_no, **unit_category (flat/shop/office)**, flat_type (1BHK/2BHK... — फक्त flat साठी), **area (sq.ft)**, price, status (available/**hold**/booked/sold), photos[]
   - **Hold/token:** customer ने token दिल्यावर unit 'hold' — hold_lead_id, hold_until (तारीख), token_amount, hold_note हे columns; मुदत संपल्यावर admin ला notification (आपोआप available होणार नाही — admin ठरवेल); booking झाल्यास token रक्कम पहिल्या payment मध्ये adjust
...
4. **leads** — ग्राहक/inquiry
   - id, name, phone, email, source, budget, status (hot/warm/cold/**lost**), **lost_reason** (lost असल्यास — यादी app_settings मधून: Budget, दुसरीकडे घेतले, Response नाही, Location आवडली नाही, Other), assigned_to (agent), notes
   - **Duplicate check:** नवीन lead टाकताना तोच phone number आधी असेल तर इशारा (जुना lead दाखवणे)
   - **source ची यादी Settings मधून** (app_settings) — defaults: Walk-in, Reference, Facebook/Instagram, Website, Hoarding, Newspaper, Other — admin ला नवीन जोडता येतील
5. **deals** — deal pipeline
   - id, lead_id, property_id, stage (inquiry/visit/negotiation/booking/sold/**cancelled**), amount, agent_id
   - **Cancellation:** deal cancel केल्यावर → unit status परत 'available', उरलेले हप्ते रद्द, refund नोंद (table #19)
6. **followups** — reminders/tasks
   - id, lead_id, **deal_id** (nullable — booking नंतरचे follow-up deal वर), due_date, note, done
   - lead_id किंवा deal_id यापैकी एक भरलेले असेल
7. **payments** — booking व installment नोंदी
   - id, deal_id, **project_id** (deal→property→project वरून auto), amount, payment_date, mode (cash/cheque/online), **installment_id** (कोणत्या हप्त्याचे — deal_installments मधून), receipt_no, notes
   - **bank_account_id** — पैसे कोणत्या project bank account मध्ये आले (त्या project च्या accounts मधून निवड)
   - **receipt_no:** प्रत्येक project ची स्वतंत्र मालिका, 1 पासून (001, 002...) — Postgres trigger (`before insert`) त्या project चा `max(receipt_no)+1` आपोआप भरेल. Receipt PDF वर project नाव + Receipt No.
...
14. **deal_charges** — final deal चे extra charges (booking wizard step 5)
    ...
    - dropdown ला default: **Parking, GST** — admin ला अजून types जोडता येतील (charge_types **app_settings** table #20 मधून)
...
16. **project_milestones** — project चे बांधकाम टप्पे (installment due ठरवण्यासाठी)
    - id, project_id, name (schedule च्या टप्प्यांची नावे — उदा. Booking, 1st Slab, Possession), status (pending/completed), completed_on
    - Project wizard step 6 मध्ये schedules तयार होताना त्यांच्या टप्प्यांमधून auto तयार
    - Projects page वर admin "**हा टप्पा पूर्ण झाला**" mark करेल → त्या project च्या सर्व bookings चा तो हप्ता due होईल
17. **deal_installments** — प्रत्येक booking चे हप्ते (tracking साठी)
    - id, deal_id, stage_name, percent, amount (final_amount × %), status (upcoming/due/partial/paid), due_date
    - Booking submit होताना assigned schedule मधून auto तयार
    - Milestone completed झाल्यावर → status='due' + notification; payment नोंदवताना हप्ता निवडणे → भरल्याप्रमाणे partial/paid
    - "किती भरले / किती बाकी" या table वरून थेट मोजता येईल
18. **company_profile** — company branding (एकच row)
    - company_name, address, phone, email, logo_url, gst_no, rera_no
    - Settings page वर admin भरेल/बदलेल; सर्व generated documents चा header इथून येईल
19. **refunds** — booking cancel झाल्यावर परताव्याची नोंद
    - id, deal_id, refund_amount, deduction_amount (कपात), refund_date, mode (cash/cheque/online), bank_account_id (कोणत्या खात्यातून परत), note
20. **app_settings** — बदलता येणाऱ्या याद्या (key-value)
    - id, key, values[] — उदा. key='lead_sources' (Walk-in, Reference, Facebook/Instagram, Website, Hoarding, Newspaper, Other), key='charge_types' (Parking, GST), key='lost_reasons' (Budget, दुसरीकडे घेतले, Response नाही, Location आवडली नाही, Other)
    - Settings page वर admin ला यादीत नवीन जोडता/काढता येईल
...
नोंद: document generate वरचे **company branding (logo/नाव/पत्ता) Settings मधील company_profile मधून येईल** (table #18) — admin ला कधीही बदलता येईल.

deals table वाढवणे: booking_date, funding_type (loan/self), assigned_schedule_id, final_amount जोडणे. शिवाय **post-booking checklist** साठी: agreement_date, registration_date, possession_date (nullable — रिकामे = तो टप्पा बाकी).
...
### Layout
- **App नाव: "RealCRM"** — sidebar logo, login page, browser tab वर हेच; generated documents वर मात्र company_profile मधले कंपनीचे खरे नाव
...
- Lead add/edit/delete form — **source dropdown app_settings (Settings) मधून**; **duplicate phone check** (तोच नंबर आधी असेल तर इशारा + जुना lead दाखवणे)
- Leads ची यादी (table) — search व filter (status नुसार)
- **Lead lost करणे** — status='lost' + कारण निवडणे (lost_reasons यादीतून); lost leads वेगळ्या filter मध्ये
- **Lead detail page (360°)** — एका page वर त्या lead चे सर्व: माहिती, follow-ups ची timeline, नंतर deal/payments/documents पण इथेच दिसतील (पुढच्या phases मध्ये विभाग वाढत जातील)
...
- Stage बदलणे: inquiry → visit → negotiation → booking → sold (+ **cancelled** — कोणत्याही stage वरून)
- **5-step Booking Wizard** (वरील तपशीलाप्रमाणे) — booking करताना
- **Kanban-style board याच phase मध्ये** — stage-नुसार columns (inquiry → visit → negotiation → booking → sold), कार्ड drag करून stage बदलणे; शिवाय table view पण (toggle)
- **Unit hold/token:** unit वर 'Hold' action — lead निवडणे, token रक्कम, कधीपर्यंत hold; मुदत संपल्यावर admin ला notification; booking केल्यास token पहिल्या payment मध्ये adjust, नाहीतर admin unit परत available करेल
- **Deal detail page** — deal चे सर्व एकत्र: customer/applicants, unit, stage, हप्ते, payments, documents, checklist (Phase 5 मध्ये विभाग वाढतील)
...
- Payment नोंदवताना **कोणत्या हप्त्याचे पैसे** हे निवडणे (deal_installments मधून) — भरल्याप्रमाणे हप्ता partial/paid होईल
- Installment नोंदी — किती भरले, किती बाकी (deal_installments table वरून थेट)
- **Receipt number:** प्रत्येक project ची स्वतंत्र मालिका 1 पासून — trigger ने auto (वर DB design #7 पहा)
- **Milestone marking:** Projects page वर admin "हा टप्पा पूर्ण झाला" (उदा. 1st Slab) mark करेल → त्या project च्या सर्व bookings चा तो हप्ता due होईल + notifications जातील
- Payment Receipt PDF तयार करणे (company_profile च्या header सह)
- **Booking cancellation flow:** deal cancel केल्यावर → unit परत 'available', उरलेले हप्ते रद्द (deal_installments cancelled), deduction वजा करून **refund नोंद** (table #19 — कोणत्या खात्यातून परत हे सह), activity_log मध्ये नोंद
- **Deal follow-ups:** booking नंतर customer वर follow-up ठेवता येईल (उदा. "उद्या पैशांबाबत फोन कर") — followups.deal_id ने; reminders त्याच pg_cron मधून
- **Post-booking checklist:** प्रत्येक deal वर Agreement / Registration / Possession — झाले की तारीख नोंदवणे; dashboard वर "किती registrations बाकी" असे आकडे
...
- **Templates चे स्वरूप:** `doc_templates.template_content` = HTML + `{{placeholder}}` format
  - Placeholders: `{{customer_name}}`, `{{co_applicant_name}}`, `{{project_name}}`, `{{wing}}`, `{{flat_no}}`, `{{flat_type}}`, `{{area}}`, `{{final_amount}}`, `{{amount_in_words}}`, `{{booking_date}}`, `{{receipt_no}}`, `{{payment_amount}}`, `{{payment_mode}}`, `{{payment_date}}`, `{{total_paid}}`, `{{balance_amount}}`, `{{company_name}}`, `{{company_address}}`, `{{company_logo}}`, `{{today_date}}`
- **5 default templates seed करणे** (English): Allotment Letter, NOC, Payment Receipt, Booking Confirmation, **Cost Sheet/Quotation**
  - **Cost Sheet:** booking आधीही देता येईल — lead + unit निवडून base price + charges (parking/GST...) + एकूण — prospect ला printable PDF
  - Settings → Templates मध्ये admin ला मजकूर बदलता येईल (textarea + शेजारी placeholder ची यादी)
- **Generate flow:** data भरून HTML → **html2pdf.js** ने PDF → Supabase Storage + documents मध्ये नोंद (source='generated')
- Header (नाव/logo/पत्ता) **company_profile** (Settings) मधून

### Phase 7 — Unit Availability Chart
- Project → tower/floor नुसार सर्व units चा visual तक्ता
- रंगाने status: हिरवा=available, **निळा=hold**, पिवळा=booked, लाल=sold
- Unit वर click केल्यास त्या property/deal ची माहिती

### Phase 8 — Global Search
- वरती एक search bar (सर्व pages वर — top bar मध्ये)
- **कशातून शोधणार:** leads (नाव/phone), applicants (नाव/phone — booking झालेले customers), properties (flat no), projects (नाव), deals — Supabase `ilike` queries नी
- Result गटानुसार (Leads / Customers / Units / Deals) dropdown मध्ये दिसतील
- Result वर click → संबंधित page वर जाणे

### Phase 9 — Dashboard/Reports + Export
- **वर 4 stat cards:** एकूण Leads (या महिन्यात नवीन किती), Active Deals, Units (available/booked/sold), Collection (जमा ₹ vs बाकी ₹)
- **Charts (Recharts library):**
  1. महिन्यानुसार bookings/sales — bar chart (गेले 12 महिने)
  2. महिन्यानुसार collection — bar/line chart
  3. Lead sources — pie chart
  4. Project-wise inventory status — stacked bar (हिरवा available / पिवळा booked / लाल sold)
- **Agent performance table:** agent, leads, deals, bookings, collection
- **"आजचे काम" panel:** आजचे followups + due हप्ते यादी
- **Filters:** project dropdown + date range — सर्व charts ना लागू
- **Reports export — नक्की 4 reports (प्रत्येक Excel + PDF मध्ये):**
  1. **Leads report** — निवडलेल्या काळातले leads (status/source/agent filter सह)
  2. **Collection report** — काळावधीतली सर्व payments (project/mode filter सह)
  3. **Outstanding report** — कोणत्या customer कडे किती बाकी (deal_installments वरून, project-wise)
  4. **Inventory report** — project-wise units व status (available/booked/sold)

### Phase 10 — Reminders / Notifications
- follow-up व installment-due वर app मध्ये bell icon + यादी
- वाचले/न-वाचले (read/unread) दाखवणे
- **कशा तयार होणार:** Supabase **pg_cron** extension — रोज सकाळी 7:00 (IST) ला SQL function `generate_daily_notifications()` चालेल:
  - आजचे/overdue followups (done=false) → assigned agent ला notification
  - due/partial हप्ते → agent + admin ला reminder (paid होईपर्यंत दर आठवड्याने पुन्हा)
  - **hold ची मुदत संपलेल्या units** → admin ला notification (unit परत available करायची की नाही हे admin ठरवेल)
  - Duplicate टाळण्यासाठी (user_id, type, related_id, due_date) वर unique constraint
- **Milestone complete झाल्यावर लगेच** (trigger ने) installment-due notifications — cron ची वाट न पाहता
- **Bell icon:** unread count badge, dropdown यादी, notification वर क्लिक → संबंधित page, "mark all read" बटण
...
तीन प्रकारचे import (प्रत्येकाला sample Excel template download करता येईल):
1. **Leads import** — columns: Name*, Phone*, Email, Source, Budget, Status (hot/warm/cold), Notes, Agent email
2. **जुन्या bookings/customers import** — आधीच झालेली bookings आणण्यासाठी — columns: Customer Name*, Phone*, Project*, Flat No*, Booking Date, Final Amount*, आतापर्यंत भरलेले (एकूण) — import झाल्यावर deal + applicant तयार होईल व भरलेली रक्कम एक payment म्हणून नोंदेल
3. **Properties/units import** — जुन्या projects च्या units साठी (wizard ऐवजी) — columns: Project*, Wing*, Floor*, Flat No*, Type, Area, Price, Status
- Import आधी **preview** — कोणत्या rows बरोबर/चुकीच्या ते दाखवणे; चुकीच्या rows वगळून बाकी import
- चुका (invalid rows) कारणासह दाखवणे व error यादी download
...
- Phase 2: नवीन lead add केला की table मध्ये दिसतो का, filter चालतो का; duplicate phone वर इशारा येतो का; lost कारणासह नोंदते का
- Phase 3: property + photo add होते का
- Phase 4: deal ची stage बदलली की save होते का; unit hold केल्यावर chart मध्ये निळी दिसते का व मुदतीनंतर notification येते का
- Phase 5: payment नोंदवले की बाकी रक्कम बरोबर दिसते का, receipt PDF निघते का
- Phase 6: document upload होते का व generate केलेली PDF (allotment/NOC/cost sheet) बरोबर data ने भरते का
- Phase 7: availability chart मध्ये unit चा रंग/status बरोबर दिसतो का; **booking cancel केल्यावर unit पुन्हा हिरवी (available) दिसते का**
...
## पुढील पाऊल
✅ Phase 0 पूर्ण (Vite + React + Tailwind + Supabase client). Planning पूर्ण final झाले आहे — आता **Phase 1 (Login/Auth)** पासून build सुरू करायचे: आधी Supabase project तयार करून `.env` मध्ये keys भरणे, मग वरील Phase 1 तपशीलाप्रमाणे. मी प्रत्येक step सोप्या मराठीत समजावत, code देत पुढे जाईन.
